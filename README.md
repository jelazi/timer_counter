...
Full setup instructions for a new PocketBase server, a new user, and a Windows release build are in [docs/POCKETBASE_SETUP.md](docs/POCKETBASE_SETUP.md).

Quick checklist:

- Deploy PocketBase with persistent `pb_data` storage.
- Import [pocketbase/pb_schema.json](pocketbase/pb_schema.json), which defines `categories`, `projects`, `tasks`, `time_entries`, `running_timers`, `monthly_targets`, and `day_overrides`.
- Create an app user in the PocketBase `users` collection.
